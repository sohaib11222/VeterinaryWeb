...
const PharmacyAdminPrescriptionRequests = () => {
  ...
  const review = async (requestId, status) => {
    try {
      ...
      toast.success(status === 'APPROVED' ? 'Prescription approved. The customer has been notified.' : 'Prescription rejected. The customer has been notified.')
      setReviewingId(null)
      setReviewNotes('')
    } catch (error) {
      toast.error(error?.message || 'Could not review prescription')
    }
  }
  ...
  return (
    <div>
      <div className="page-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          <h3 className="page-title mb-1">Prescription Requests</h3>
          <div className="text-muted">Review customer prescriptions before enabling purchase of prescription-only medicines.</div>
        </div>
      ...
      <div className="card">
        <div className="card-body">
          {requestsQuery.isLoading ? (
            ...
            <div className="text-center py-5"><i className="fa-solid fa-file-prescription text-muted" style={{ fontSize: 34 }}></i><h5 className="mt-3">No prescription requests</h5><p className="text-muted mb-0">New customer submissions will appear here automatically.</p></div>
          ) : (
                  ...
                  <article className="border rounded-3 p-3" key={request?._id}>
                    <div className="d-flex justify-content-between align-items-start gap-3 flex-wrap">
                      <div>
                        ...
                        <div className="text-muted small mt-1">Variant: {getVariantName(request)}</div>
                        <div className="text-muted small mt-1">Submitted {request?.createdAt ? new Date(request.createdAt).toLocaleString() : 'recently'}</div>
                      </div>
                    ...
                    <div className="d-flex align-items-center justify-content-between gap-3 flex-wrap mt-3 pt-3 border-top">
                      <a className="btn btn-outline-primary btn-sm" href={fileUrl} target="_blank" rel="noreferrer">
                        <i className="fa-solid fa-arrow-up-right-from-square me-2"></i>Open prescription
                      </a>
                      {isPending && !isReviewing && <button type="button" className="btn btn-primary btn-sm" onClick={() => startReview(request)}>Review request</button>}
                      {!isPending && request?.reviewNotes && <div className="text-muted small"><strong>Review note:</strong> {request.reviewNotes}</div>}
                    </div>

                    {isReviewing && (
                      <div className="mt-3 p-3 rounded-3" style={{ background: '#f8fafc' }}>
                        <label className="form-label">Review note {`(shown to the customer if rejected)`}</label>
                        <textarea className="form-control" rows={3} value={reviewNotes} onChange={(event) => setReviewNotes(event.target.value)} placeholder="Add an optional note for the customer." />
                        <div className="d-flex justify-content-end gap-2 mt-3 flex-wrap">
                          <button type="button" className="btn btn-light" onClick={() => setReviewingId(null)} disabled={reviewMutation.isPending}>Cancel</button>
                          <button type="button" className="btn btn-outline-danger" onClick={() => review(request._id, 'REJECTED')} disabled={reviewMutation.isPending}>Reject</button>
                          <button type="button" className="btn btn-success" onClick={() => review(request._id, 'APPROVED')} disabled={reviewMutation.isPending}>Approve & notify customer</button>
                        </div>
                      </div>
                    )}
                  </article>
