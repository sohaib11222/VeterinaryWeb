    ...
    <section className="veterinary-dashboard patient-invoices-mobile">
      <div className="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
        <div className="veterinary-dashboard-header mb-0">
          <h2 className="dashboard-title"><i className="fa-solid fa-file-invoice-dollar me-3" />Invoices</h2>
          <p className="dashboard-subtitle">Payment records for your veterinary appointments.</p>
        </div>
      ...
      <div className="dashboard-card veterinary-card mb-4"><div className="dashboard-card-body"><div className="row g-3">
        <div className="col-xl-5 col-lg-6"><label className="visually-hidden" htmlFor="patient-invoice-search">Search invoices</label><div className="input-group"><span className="input-group-text bg-white"><i className="fa-solid fa-magnifying-glass text-muted" /></span><input id="patient-invoice-search" type="search" className="form-control" value={search} onChange={(event) => updateFilter(() => setSearch(event.target.value))} placeholder="Search veterinarian, pet, appointment, or status" />{search && <button type="button" className="btn btn-outline-secondary" onClick={() => updateFilter(() => setSearch(''))}>Clear</button>}</div></div>
        <div className="col-xl-2 col-lg-2"><label className="visually-hidden" htmlFor="patient-invoice-status">Payment status</label><select id="patient-invoice-status" className="form-select" value={status} onChange={(event) => updateFilter(() => setStatus(event.target.value))}><option value="">All statuses</option><option value="SUCCESS">Successful</option><option value="PENDING">Pending</option><option value="FAILED">Failed</option><option value="REFUNDED">Refunded</option></select></div>
        <div className="col-xl-2 col-lg-2"><label className="visually-hidden" htmlFor="patient-invoice-from">From date</label><input id="patient-invoice-from" type="date" className="form-control" value={fromDate} onChange={(event) => updateFilter(() => setFromDate(event.target.value))} /></div>
        <div className="col-xl-2 col-lg-2"><label className="visually-hidden" htmlFor="patient-invoice-to">To date</label><input id="patient-invoice-to" type="date" className="form-control" value={toDate} onChange={(event) => updateFilter(() => setToDate(event.target.value))} /></div>
      ...
      <div className="dashboard-card veterinary-card"><div className="dashboard-card-body p-0">
        {query.isLoading ? <div className="text-center py-5"><div className="spinner-border text-primary" role="status"><span className="visually-hidden">Loading invoices</span></div></div>
          : query.isError ? <div className="alert alert-danger m-3">{query.error?.message || 'Unable to load invoices.'}</div>
          ...
          : <div className="p-3 d-grid gap-3">{invoices.map((invoice) => {
            ...
            return <article key={invoice._id} className="border rounded-3 p-3 bg-white"><div className="row g-3 align-items-center">
              ...
              <div className="col-xl-3 col-lg-4"><div className="text-muted small mb-1">VETERINARIAN & PET</div><div className="fw-semibold">{veterinarian?.name || veterinarian?.fullName || veterinarian?.email || '—'}</div><small className="text-muted">{pet?.name ? `${pet.name}${pet.species ? ` · ${pet.species}` : ''}` : 'Pet unavailable'}</small></div>
              ...
              <div className="col-xl-2 col-lg-12"><div className="d-flex justify-content-xl-end gap-2 patient-invoice-actions"><Link to={`/patient-invoices/${encodeURIComponent(String(invoice._id))}`} className="btn btn-outline-primary btn-sm"><i className="fa-solid fa-eye me-1" />View</Link><button type="button" className="btn btn-primary btn-sm" onClick={() => download(invoice)} disabled={isDownloading}><i className={`fa-solid ${isDownloading ? 'fa-spinner fa-spin' : 'fa-download'} me-1`} />{isDownloading ? 'Preparing…' : 'PDF Download'}</button></div></div>
            </div></article>
          })}</div>}
      </div></div>

      {(pagination.pages || 1) > 1 && <nav className="d-flex justify-content-center gap-2 mt-4 patient-invoice-pagination" aria-label="Invoice pages"><button type="button" className="btn btn-outline-primary" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Previous</button><span className="align-self-center text-muted">Page {page} of {pagination.pages}</span><button type="button" className="btn btn-outline-primary" disabled={page >= pagination.pages} onClick={() => setPage((current) => current + 1)}>Next</button></nav>}
    </section>
  )
}
