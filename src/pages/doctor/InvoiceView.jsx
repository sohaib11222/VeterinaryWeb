...
const InvoiceView = () => {
  ...
  if (invoiceQuery.isLoading) return <div className="text-center py-5"><div className="spinner-border text-primary" role="status"><span className="visually-hidden">Loading invoice</span></div></div>
  if (invoiceQuery.isError || !invoice?._id) return <div className="alert alert-danger">{invoiceQuery.error?.message || 'Invoice not found.'}</div>
  ...
  return <section className="veterinary-dashboard"><div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4"><Link to="/invoices" className="btn btn-outline-secondary"><i className="fa-solid fa-arrow-left me-2" />Back to invoices</Link><button type="button" className="btn btn-primary" onClick={download} disabled={isDownloading}><i className={`fa-solid ${isDownloading ? 'fa-spinner fa-spin' : 'fa-download'} me-2`} />{isDownloading ? 'Preparing PDF…' : 'Download PDF'}</button></div>
    <article className="card border-0 shadow-sm overflow-hidden"><div className="card-body p-0"><header className="p-4 p-lg-5" style={{ background: 'linear-gradient(135deg, #075e54, #118b80)', color: '#fff' }}><div className="row align-items-start g-3"><div className="col-md-7"><div className="text-uppercase small opacity-75 mb-2">MyPetPlus · Veterinary care</div><h1 className="h2 mb-2">Invoice</h1><p className="mb-0 opacity-75">{appointment?.appointmentNumber || `Transaction ${String(invoice._id).slice(-8).toUpperCase()}`}</p></div><div className="col-md-5 text-md-end"><span className={`badge fs-6 ${status === 'SUCCESS' ? 'text-bg-light text-success' : 'text-bg-warning'}`}>{status}</span><div className="display-6 fw-bold mt-3">{money(invoice?.amount, invoice?.currency)}</div></div></div></header>
      <div className="p-4 p-lg-5"><div className="row g-4 mb-5"><section className="col-md-6"><h2 className="h6 text-uppercase text-muted mb-3">Pet owner</h2><div className="fw-semibold fs-5">{petOwner?.name || petOwner?.fullName || petOwner?.email || '—'}</div><div className="text-muted">{petOwner?.email || 'No email on file'}</div></section><section className="col-md-6"><h2 className="h6 text-uppercase text-muted mb-3">Patient</h2><div className="fw-semibold fs-5">{pet?.name || '—'}</div><div className="text-muted">{[pet?.species, pet?.breed].filter(Boolean).join(' · ') || 'Pet details unavailable'}</div></section></div>
