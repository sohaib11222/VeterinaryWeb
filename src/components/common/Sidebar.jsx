...
const Sidebar = ({ userType = 'patient' }) => {
  ...
  if (userType === 'pet_sitter') {
    ...
    return <div className="profile-sidebar veterinary-sidebar"><div className="widget-profile veterinary-profile-widget"><div className="profile-info-widget"><Link to="/pet-sitter/profile" className="booking-doc-img"><img src={profileImage} alt={t('petSitterPanel.nav.role')} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = '/assets/img/doctors-dashboard/doctor-profile-img.jpg' }} /></Link><div className="profile-det-info"><h3><Link to="/pet-sitter/profile">{displayName}</Link></h3><span className="badge veterinary-role-badge"><i className="fa-solid fa-paw me-1" />{t('petSitterPanel.nav.role')}</span></div></div></div><div className="dashboard-widget veterinary-dashboard-menu"><nav className="dashboard-menu"><ul><li className={isActive('/pet-sitter/dashboard') ? 'active' : ''}><Link to="/pet-sitter/dashboard"><i className="fa-solid fa-shapes" /><span>{t('petSitterPanel.nav.dashboard')}</span></Link></li><li className={isActive('/pet-sitter/profile') ? 'active' : ''}><Link to="/pet-sitter/profile"><i className="fa-solid fa-user-pen" /><span>{t('petSitterPanel.nav.profile')}</span></Link></li><li className={isActive('/pet-sitter/services') ? 'active' : ''}><Link to="/pet-sitter/services"><i className="fa-solid fa-handshake" /><span>Services I Offer</span></Link></li><li className={isActive('/pet-sitter/chats') ? 'active' : ''}><Link to="/pet-sitter/chats"><i className="fa-solid fa-comments" /><span>{t('petSitterPanel.nav.chats')}</span>{unreadAdminMessages > 0 && <small className="unread-msg veterinary-badge">{unreadAdminMessages}</small>}</Link></li><li className={isActive('/pet-sitter/support-tickets') ? 'active' : ''}><Link to="/pet-sitter/support-tickets"><i className="fa-solid fa-headset" /><span>{t('petSitterPanel.nav.supportTickets')}</span>{unreadSupportTickets > 0 && <small className="unread-msg veterinary-badge">{unreadSupportTickets}</small>}</Link></li><li className={isActive('/pet-sitter/change-password') ? 'active' : ''}><Link to="/pet-sitter/change-password"><i className="fa-solid fa-key" /><span>{t('petSitterPanel.nav.changePassword')}</span></Link></li></ul></nav></div></div>
  }
        ...
        <div className="dashboard-widget veterinary-dashboard-menu">
          <nav className="dashboard-menu">
            <ul>
              ...
              <li className={isActive('/pharmacy-admin/services') ? 'active' : ''}>
                <Link to="/pharmacy-admin/services">
