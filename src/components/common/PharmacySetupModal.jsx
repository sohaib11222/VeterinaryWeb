...
const PharmacySetupModal = ({ setup, role }) => {
  ...
  const accountLabel = isParapharmacy ? t('pharmacyAdmin.setup.parapharmacy') : t('pharmacyAdmin.setup.pharmacy')
          ...
          <div className="modal-content border-0 shadow-lg overflow-hidden">
            <div className="p-4 text-white" style={{ background: 'linear-gradient(135deg, #0b5d5b, #118f8a)' }}>
              <div className="d-flex align-items-center gap-3">
                ...
                <div><h4 className="mb-1">{t('pharmacyAdmin.setup.finish', { account: accountLabel })}</h4><div className="small" style={{ opacity: .9 }}>{t('pharmacyAdmin.setup.description')}</div></div>
              </div>
            </div>
              ...
              <div className="d-grid gap-3">
                {requirements.map((requirement) => (
                  <div key={requirement.key} className="d-flex align-items-center gap-3 border rounded-3 p-3">
                    ...
                    <div className="flex-grow-1"><div className="fw-semibold">{requirement.label}</div><div className={`small ${requirement.complete ? 'text-success' : 'text-muted'}`}>{requirement.complete ? t('pharmacyAdmin.setup.completed') : t('pharmacyAdmin.setup.required')}</div></div>
                    {!requirement.complete && <Link to={requirement.key === 'subscription' ? '/pharmacy-admin/subscription' : '/pharmacy-admin/profile'} className="btn btn-sm btn-outline-primary">{t('pharmacyAdmin.setup.open')}</Link>}
                  </div>
                ))}
              </div>
              <div className="alert alert-light border mt-4 mb-0 small"><i className="fa-solid fa-circle-info me-2 text-primary"></i>{t('pharmacyAdmin.setup.unlockMessage')}</div>
            </div>
          </div>
        </div>
      </div>
